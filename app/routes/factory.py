...
@cache.cached(timeout=300,
              key_prefix=lambda: f"factory-dash-{current_user.id}")
...
def create_vouchers():
    ...
    if form.validate_on_submit():
        ...
        batch = VoucherBatch(
            ...
            product    = product,
            status     = 'under_review'   # أول حالة
        )
        db.session.add(batch)
        db.session.flush()   # للحصول على batch.id قبل إنشاء السندات

        # ❷ أنشئ السندات واربطها بالدفعة
        for _ in range(count):
            code        = Voucher.generate_unique_code()
            total_price = (price_per_unit * quantity) * (1 + vat_rate)
            ...
            v = Voucher(
                ...
                factory_commission = commission_per * quantity,
                factory_id         = factory.id,
                batch_id           = batch.id,          # <— الربط الجديد
                city               = city  # ← جديد
            )
            db.session.add(v)
            db.session.flush()

            # توليد QR وربط المسار
            v.qr_path = generate_qr_code(code)
            created.append(v)

        db.session.commit()
        flash(f'✔ تم إصدار {len(created)} سندًا في دفعة واحدة (Batch ID: {batch.id})', 'success')
    ...
    form=form,
    created=created,
    active_tab="issue",                # ← يحدد التبويب النشط
...
# routes/factory_bp.py
@factory_bp.route('/commission-payments/<int:batch_id>/upload',
                  methods=['GET', 'POST'])
@login_required
def upload_batch_proof(batch_id):
    """
    GET   → يعرض صفحة إدخال اسم الحساب البنكي
    POST  → يستقبل الاسم، يغيّر حالة الدفعة إلى payment_under_review
    """
    batch = VoucherBatch.query.filter_by(
        id=batch_id,
        factory_id=current_user.factory_id
    ).first_or_404()

    # ---------- POST: معالجة تأكيد الدفع ----------
    if request.method == 'POST':
        account_name = request.form['account_name'].strip()

        # يسمح بالرفع إذا كانت الدفعة awaiting_payment أو payment_rejected
        ...
        for v in batch.vouchers:
            v.status = 'payment_under_review'

        db.session.commit()
        flash("✔ تم رفع بيانات التحويل، بانتظار اعتماد الإدارة.", "success")
        return redirect(url_for('factory.commission_payments'))

    # ---------- GET: عرض صفحة رفع الإثبات ----------
    if batch.status not in ['awaiting_payment', 'payment_rejected']:
        flash("⚠️ لا يمكن رفع إثبات لهذه الدفعة في وضعها الحالي.", "warning")
        return redirect(url_for('factory.commission_payments'))

    return render_template(
        'dashboard_factory/upload_batch_proof.html',
        batch=batch,
        active_tab='payments'
    )
...
def vouchers_list():
    factory_id = current_user.factory_id
    vouchers = Voucher.query.filter_by(factory_id=factory_id).order_by(Voucher.created_at.desc()).all()
    return render_template('dashboard_factory/vouchers_list.html', vouchers=vouchers)
