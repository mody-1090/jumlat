...
def _download_image_to_reader(url):
    """
    تحميل الصورة من رابط عام وتحويلها إلى ImageReader
    """
    if not url:
        return None
    ...
    try:
        ...
        current_app.logger.warning(f"تعذر تحميل صورة المنتج من الرابط: {e}")
        return None
...
def _draw_product_details_page(c, voucher, font):
    ...
    if getattr(voucher, "product_description", None):
        ...
        c.roundRect(MARGIN, current_y - desc_box_height + 8, w - 2 * MARGIN, desc_box_height, 8, fill=1, stroke=1)
    ...
    if getattr(voucher, "product_image_url", None):
        ...
        if img_reader:
            ...
            c.setFillColor(colors.red)
            c.setFont(font, 11)
            c.drawRightString(w - MARGIN, current_y, ar("تعذر تحميل صورة المنتج من الرابط"))
            current_y -= 18
